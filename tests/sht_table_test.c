...
void test_SHT_CreateFile(void) {
	BF_Init(LRU);
	SHT_CreateIndex(FILE_NAME,10);
    // Check if the file is created.
    TEST_CHECK((access(FILE_NAME, F_OK) == 0));
    // Open the file to get the struct with the metadata.
    SHT_info* info = SHT_OpenFile(FILE_NAME);
...
TEST_LIST = {
	{ "SHT_CreateFile", test_SHT_CreateFile },
	// { "SHT_OpenFile", test_SHT_OpenFile },
	// { "SHT_InsertEntry\n     SHT_GetAllEntries", test_SHT_Insert_HT_Get},
